...
router.get("/usage-stats", auth, async (req, res) => {
  try {
    ...
    // Get active projects count (all projects for this user)
    const activeProjects = await Project.countDocuments({ user: user._id });
...
// @route   PATCH /api/users/projects/:id/status
// @desc    Change project status (active, archived, completed)
// @access  Private
